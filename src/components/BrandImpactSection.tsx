    ...
    function draw() {
      ...
      for (let i = 0; i < particles.length; i++) {
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (d < 110) {
            ctx.strokeStyle = `rgba(0, 229, 255, ${(1 - d / 110) * .05})`;
            ctx.lineWidth = .5;
          ...
          <span key={wi}>
            <span style={{ display: 'inline-block', whiteSpace: 'nowrap', position: 'relative' }}>
              {word.split('').map((char, ci) => {
                ...
                let display = char, color = isHL && local <= 0.05 ? '#00E5FF' : '#fff';
                let ty = 0, blur = 0, scaleY = 1, op = 1;
                let shadow = isHL && local <= 0.05 ? '0 0 20px rgba(0, 229, 255, 0.4)' : 'none';
                ...
                if (local > 0.05) {
                  ...
                  if (direction === 'out') {
                    ...
                    if (local > 0.6) {
                      ...
                      display = GLYPHS[~~(Math.random() * GLYPHS.length)];
                      color = `rgba(0, 229, 255, ${0.3 + d * 0.4})`;
                      ty = -(d * 8); blur = d * 3; op = 0.5 + (1 - d) * 0.3; scaleY = 1 + d * 0.15;
                    } else if (local > 0.15) {
                      const d = (local - 0.15) / 0.45;
                      display = d < 0.7 ? GLYPHS[~~(Math.random() * GLYPHS.length)] : char;
                      color = isHL && d >= 0.7 ? '#00E5FF' : `rgba(0, 229, 255, ${0.7 - d * 0.5})`;
                      ty = -(1 - d) * 5; blur = (1 - d) * 2;
                      shadow = isHL && d >= 0.7 ? '0 0 15px rgba(0, 229, 255, 0.3)' : 'none';
                    } else {
                      display = char;
                      color = isHL ? '#00E5FF' : '#fff';
                      shadow = isHL ? '0 0 20px rgba(0, 229, 255, 0.4)' : 'none';
                    }
                  }
                }
              ...
              {isHL && (
                <span style={{
                  ...
                  boxShadow: '0 0 10px rgba(0, 229, 255, 0.5)', borderRadius: '1px',
                }} />
              )}
            </span>
      ...
      >
        ...
        >
          <motion.div
            ...
            style={{ background: '#00E5FF', boxShadow: '0 0 15px #00E5FF, 0 0 40px rgba(0, 229, 255, 0.4)' }}
          />
          <motion.div
    ...
    >
      ...
      >
        <motion.div
          ...
          style={{ background: 'conic-gradient(from 0deg, transparent 0%, rgba(0, 229, 255, 0.3) 25%, transparent 50%, rgba(161,0,255,0.2) 75%, transparent 100%)' }}
        />
        <motion.div
